...
def parse_pdf_document(pdf_path, max_shapes_threshold=250):
    laparams = LAParams(line_margin=0.2, word_margin=0.1, boxes_flow=None)

    # 1. Pre-flight Check: Count vector shapes on Page 1 with Early Exit
    page1_generator = extract_pages(pdf_path, maxpages=1, laparams=laparams)
    first_page = next(page1_generator, None)

    if first_page:
        shape_count = 0
        limit_exceeded = False

        # Custom exception to abort traversal immediately
        class ShapeLimitExceeded(Exception):
            pass

        def count_page_shapes(element):
            nonlocal shape_count
            if isinstance(element, (LTRect, LTCurve, LTLine)):
                shape_count += 1
                if shape_count > max_shapes_threshold:
                    raise ShapeLimitExceeded()

            if isinstance(element, LTContainer):
                for child in element:
                    count_page_shapes(child)

        try:
            for element in first_page:
                count_page_shapes(element)
        except ShapeLimitExceeded:
            limit_exceeded = True
        ...
        if limit_exceeded:
            return json.dumps({
                "success": False,
                "is_rejected": True,
                "reason": "RASTERIZED_VECTOR_LIMIT_EXCEEDED",
                "shape_count": shape_count,
                "max_threshold": max_shapes_threshold,
                "message": f"Page 1 contains over {max_shapes_threshold} vector shapes. Rejection triggered early at shape count {shape_count}."
            })

    # 2. Main Extraction Loop (Only reached if pre-flight check passes)
    parsed_pages = []
    current_y_offset = 0.0
...
# Execution
parse_pdf_document("input_isp.pdf")
`;
        ...
        // Pre-flight Guard: Reject rasterized/dense vector PDFs
        if (parsedResult.is_rejected) {
            status.innerText = `Error: Unable to process PDF - this appears to be a scanned PDF (not a "Text" PDF).`;
            throw new Error(
                `Unable to process PDF: Document contains too many shapes or images on page 1 ` +
                `(limit is ${parsedResult.max_threshold}). Please upload a text-based PDF.`
            );
        }

        // Extract the valid pages array from the payload data
        const rawParsedPages = parsedResult.data;
